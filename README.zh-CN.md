...
- 每个项目是做什么的？
- 哪个项目更适合我的环境？
- OpenClaw、IronClaw、PicoClaw、MimiClaw、LocalClaw、Mimiclaw 之间有什么区别？
- 哪些适合自托管？哪些适合本地推理？哪些更偏硬件？哪些更重视安全？哪些适合多 Agent？
...
- 选择 **[IronClaw](https://github.com/nearai/ironclaw)**：如果你想要一个基于 Rust、强调隐私和安全的 OpenClaw 风格实现
...
- **[IronClaw](https://github.com/nearai/ironclaw)** — 基于 Rust、强调隐私与安全的 OpenClaw 风格实现
...
| [IronClaw](https://github.com/nearai/ironclaw) | 安全优先助手运行时 | 重视隐私和安全的自托管用户 | 本地机器 / 服务器 | Rust 实现、本地加密存储、WASM 沙箱、多层防护 | 活跃但较新 |
...
- 本地开发机
- 自托管服务器
- 带 PostgreSQL + pgvector 的部署环境

**为什么重要：**
IronClaw 代表了 Claw 生态中更偏隐私与安全的路线。它强调本地加密存储、透明设计、WASM 沙箱、prompt injection 防护、端点白名单和可扩展插件式能力。
...
- **技术栈**：TypeScript、Go、C、Rust、本地模型栈等
- **部署门槛**：桌面、服务器、Docker、嵌入式开发板、本地 LLM 运行时、基于 PostgreSQL 的部署
- **交互方式**：Telegram、Slack、WhatsApp、CLI、Web、Matrix 等
- **适合场景**：个人自动化、隐私优先、本地推理、安全优先自托管、边缘部署、硬件助手、多 Agent 协作
- **成熟度**：稳定、活跃、早期、实验性
...
- 想做项目选型的用户
- 想跟踪生态分化方向的贡献者
- 想比较自托管、本地优先、嵌入式、安全优先、多 Agent 路线差异的人
